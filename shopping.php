...
a{color:#1769d2;text-decoration:none}.top{background:#1f7ae0;color:#fff}.top-inner{max-width:1120px;margin:0 auto;padding:18px 16px;display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:20px;font-weight:700}.nav a{color:#fff;margin-left:16px}.wrap{max-width:1120px;margin:24px auto;padding:0 16px}.hero{background:#fff;border:1px solid #e5e8ef;border-radius:6px;padding:22px;margin-bottom:18px}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:16px}.card{background:#fff;border:1px solid #e5e8ef;border-radius:6px;overflow:hidden;display:flex;flex-direction:column;min-width:0}.thumb{aspect-ratio:16/9;background:#eef2f7;display:flex;align-items:center;justify-content:center;color:#687385;overflow:hidden}.thumb img{width:100%;height:100%;object-fit:contain}.body{padding:14px;display:flex;flex:1;flex-direction:column}.body h2{font-size:18px;line-height:1.35;margin:0 0 10px}.card-action{margin-top:auto;padding-top:8px}.price{font-size:20px;color:#b42318;font-weight:700}.muted{color:#687385}.btn,.button{box-sizing:border-box;display:inline-flex;align-items:center;justify-content:center;min-height:44px;border:0;background:#1f6fc2;color:#fff;border-radius:4px;padding:9px 14px;cursor:pointer}.btn:focus-visible,.button:focus-visible,.nav a:focus-visible{outline:3px solid #111827;outline-offset:3px}.btn-default{background:#eef2f7;color:#1f2933}.form{background:#fff;border:1px solid #e5e8ef;border-radius:6px;padding:18px}.query-panel{max-width:640px}.field{margin-bottom:13px}.field label{display:block;font-weight:600;margin-bottom:6px}.field-help{display:block;color:#687385;font-size:13px;line-height:1.55;margin:-3px 0 9px}.field input,.field textarea,.field select{box-sizing:border-box;width:100%;min-height:44px;border:1px solid #9aa4b2;border-radius:4px;padding:10px;font-size:16px}.detail{display:grid;grid-template-columns:minmax(260px,420px) 1fr;gap:20px}.detail-img{background:#eef2f7;border:1px solid #e5e8ef;border-radius:6px;min-height:260px;display:flex;align-items:center;justify-content:center;overflow:hidden}.detail-img img{width:100%;height:100%;max-height:420px;object-fit:contain}.detail-next{padding:12px 14px;border-left:4px solid #1f6fc2;background:#f4f8fd}.checkout-summary{display:flex;align-items:center;gap:10px;margin-bottom:16px}.checkout-product-icon{display:block;width:36px;height:36px;flex:0 0 36px;object-fit:contain;border-radius:4px}.checkout-order{min-width:0}.checkout-order-label{font-size:12px;color:#687385;margin-bottom:3px}.checkout-order-no{font-weight:600;overflow-wrap:anywhere}.pay-panel{background:#fff;border:1px solid #e5e8ef;border-radius:6px;padding:18px}.pay-options{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-top:14px}.pay-option{box-sizing:border-box;display:flex;align-items:center;justify-content:center;gap:9px;min-height:48px;border:1px solid #7897bc;border-radius:4px;background:#fff;color:#145ea8;font-weight:600;padding:10px 14px}.pay-option:hover{border-color:#1f6fc2;background:#f4f8fd}.pay-option img{width:24px;height:24px;object-fit:contain}.table{width:100%;border-collapse:collapse;background:#fff}.table th,.table td{border:1px solid #d5dbe4;padding:10px;text-align:left}.footer{text-align:center;color:#687385;padding:30px 16px}@media(max-width:720px){.top-inner{display:block}.nav{margin-top:8px}.nav a{display:inline-block;margin:0 12px 6px 0;padding:5px 0}.detail{grid-template-columns:1fr}.pay-options{grid-template-columns:1fr}.wrap{margin-top:16px;padding:0 12px}.hero,.form{padding:16px}}
	</style></head><body><div class="top"><div class="top-inner"><div class="brand">'.shop_h($config['shop_name']).'</div><nav class="nav" aria-label="商城导航"><a href="shopping.php">商城展示</a><a href="shopping.php?act=query">订单查询</a><a href="/">返回首页</a></nav></div></div><main class="wrap">';
}
...
if ($act === 'checkout') {
    ...
    echo '</div><div class="form"><h1>'.shop_h($goods['name']).'</h1><p class="muted">'.nl2br(shop_h($goods['description'])).'</p><div class="detail-next"><strong>如何购买</strong><p>购买入口由发起订单的商户页面提供，具体服务内容与金额以实际订单为准。</p></div><p><a class="btn btn-default" href="shopping.php">返回商城展示</a></p>';
    echo '</div></div>';
} elseif ($act === 'query') {
    ...
    echo '<div class="hero query-panel"><h1>订单查询</h1><p id="query-auth-help" class="field-help">输入商城订单号，并填写下单后获得的查询凭证；如果没有查询凭证，可改用下单手机号后四位。</p><form method="post" action="shopping.php?act=query" class="form" style="padding:0;border:0">
      <div class="field"><label for="shop-trade-no">商城订单号</label><input id="shop-trade-no" type="text" name="trade_no" value="'.shop_h($tradeNo).'" placeholder="S 开头的商城订单号" autocomplete="off" required></div>
      <div class="field"><label for="shop-query-token">查询凭证</label><input id="shop-query-token" type="text" name="token" value="'.shop_h($token).'" placeholder="下单后生成的查询凭证" aria-describedby="query-auth-help" autocomplete="off"></div>
      <div class="field"><label for="shop-contact-last4">手机号后四位</label><input id="shop-contact-last4" type="text" name="contact_last4" value="'.shop_h($last4).'" placeholder="没有查询凭证时填写" aria-describedby="query-auth-help" inputmode="numeric" pattern="[0-9]{4}" maxlength="4" autocomplete="off"></div>
    ...
    echo '<div class="hero"><h1>服务与商品</h1><p class="muted">'.shop_h($config['shop_desc']).'</p></div>';
    if (!\lib\Shop\ConfigService::isEnabled()) {
        ...
        if (empty($goodsList)) {
            ...
            foreach ($goodsList as $goods) {
                ...
                echo '</div><div class="body"><h2>'.shop_h($goods['name']).'</h2><p class="muted">'.shop_h($summary).'</p><p class="card-action"><a class="btn" href="shopping.php?act=detail&amp;id='.intval($goods['id']).'">查看详情</a></p></div></div>';
            }
            echo '</div>';
        }
    }
