    ...
    private function updateInfo($info){
        ...
        if($row){
            ...
            if($order || $conf['complain_range']==1){
                ...
                $DB->insert('complain', ['paytype'=>$order['type'], 'channel'=>$order['channel'], 'subchannel'=>$subchannel, 'uid'=>$order['uid'] ?? 0, 'trade_no'=>$trade_no, 'thirdid'=>$thirdid, 'type'=>$type, 'title'=>'微信交易投诉('.$info['complaintCount'].'次)', 'content'=>$info['content'], 'status'=>$status, 'phone'=>$info['payerPhone'], 'addtime'=>$time, 'edittime'=>$time, 'thirdmchid'=>$info['channelMerchantNo']]);
