...
class FuiouWxpay implements IComplain
{
    ...
    function __construct($channel){
        ...
        $this->service = new \EntryService($channel['appid'],$channel['entrykey']);
	}
    ...
    private function updateInfo($info){
        ...
        if(!$row){
            ...
            if(!$order){
                $order = $DB->find('order', 'trade_no,uid,channel,subchannel', ['api_trade_no'=>$api_trade_no]);
                if($order){
