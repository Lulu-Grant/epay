...
?>
 <div id="content" class="app-content" role="main">
    <div class="app-content-body ">
		<div class="modal inmodal fade" id="myModal" tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="welcome-dialog-title" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">关闭</span>
						</button>
						<h4 class="modal-title" id="welcome-dialog-title">欢迎回来</h4>
					</div>
...
<div id="dashboard-data-status" class="sr-only" role="status" aria-live="polite">首页统计加载中</div>
<!-- stats -->
<?php
...
?>
          ...
          <div class="row row-sm text-center merchant-stat-grid">
            <div class="col-xs-6 col-sm-3">
              <div class="panel padder-v item merchant-stat-card">
			  ...
			  <div class="col-xs-9"><div class="h1 text-primary-dk font-thin h1 merchant-stat-value"><span class="text-muted text-md">￥</span><?php echo $userrow['money']?></div><span class="text-muted">商户当前余额</span></div>
			  </div>
			  </div>
            </div>
			<div class="col-xs-6 col-sm-3">
			  ...
			  <div class="col-xs-9"><div class="h1 text-dark-dk font-thin h1 merchant-stat-value"><span class="text-muted text-md">￥</span><span id="settle_money">加载中</span></div><span class="text-muted">已结算余额</span></div>
			  </div>
			  </div>
            </div>
			<div class="col-xs-6 col-sm-3">
			  ...
			  <div class="col-xs-9"><div class="h1 text-success-dk font-thin h1 merchant-stat-value"><span id="orders">加载中</span><span class="text-muted text-md">个</span></div><span class="text-muted">订单总数</span></div>
			  </div>
			  </div>
            </div>
			<div class="col-xs-6 col-sm-3">
			  ...
			  <div class="col-xs-9"><div class="h1 text-info-dk font-thin h1 merchant-stat-value"><span id="orders_today">加载中</span><span class="text-muted text-md">个</span></div><span class="text-muted">今日订单</span><div class="text-muted text-sm m-t-xs">昨日：<span id="orders_lastday">加载中</span> 个</div></div>
			  </div>
			  </div>
            </div>
        </div>
...
<?php }?>
          ...
          <div class="panel wrapper">
            ...
            <?php }else{?>
            <div class="dashboard-empty-state" role="status"><strong>暂无结算记录</strong><p>完成结算后，这里会显示最近的结算趋势。</p></div>
            <?php }?>
          </div>
        </div>
      </div>
...
$(document).ready(function(){
	$.ajax({
		...
		success : function(data) {
			...
			$('#dashboard-data-status').text('首页统计已更新');
		},
		error : function() {
			$('#orders,#orders_today,#orders_lastday,#settle_money').text('加载失败');
			$('#dashboard-data-status').text('首页统计加载失败，请刷新页面重试。').addClass('text-danger');
		}
	});
